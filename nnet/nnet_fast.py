...
class NeuralNetwork (object):
    ...
    # Note: fDif est ignoré dans cette version (inutile)
    def train(self, train_set, n_epoch, batch_size=1, fDif=False, valid_set=None, test_set=None, EarlyStopping=False, filename=""):
        ...
        for k in xrange(n_epoch):
            ...
            # Affichage
            print "Epoch %4d" % k, "- Temps de calcul = %.3fs" % float(t2-t1), "- Cout optimisé total = %.3f" % L_total
            print "Train:\tErreur de classification = %3.1f" % (Err*100.),       "%" + " - Cout moyen = %.4f" % R
