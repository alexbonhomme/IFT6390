...
\begin{align}
  X_N = USU^T
\end{align}
Ou les colonnes de $U$ contienent les vecteurs propres et $S^2$ les valeurs propres associées. On ne conserve alors qu'une nombre $M$ de vecteurs propres (ie. $M$ colonnes de la matrice $U$). Après réduction du nombre de colonne on obtient une matrice $U^{\prime}$ (aussi appelée, dans notre cas, <<Eigenspace>>) dans laquelle on projetent l'ensemble de donnée $X$ afin de déterminer la matrice de poids $W$ associée :
\begin{align}
